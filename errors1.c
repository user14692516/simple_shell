...
/**
 * _erratoi - This one converts a string to an integer
...
int _erratoi(char *s)
{
	...
	if (*s == '+')
		s++;  /* Why does this make the main return 255? */
	for (i = 0;  s[i] != '\0'; i++)
	{
		if (s[i] >= '0' && s[i] <= '9')
		{
 ...
 * @info: info is the parameter & return info struct
 * @estr: estr is the string containing specified error type
 * Return: It returns 0 if no numbers in string, converted number otherwise
 *        -1 on error
 */
 ...
 *
 * Return: It will return number of characters printed
 */
...
/**
 * convert_number - This is the converter function.
 * @num: This is the number
 * @base: This one is the base
